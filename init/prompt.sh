...
function _ps1_status()
{
	...
	if [[ "$STATUS" -eq "0" ]]; then
		printf "%s" "${C_LGREEN}0${C_RESET}"
	else
		printf "%s" "${C_LGREEN}${STATUS}${C_RESET}"
	fi
};
...
# Bash
export PROMPT_COMMAND='export PS1="`_ps1_status` ${C_CYAN}\h ${C_LGREEN}\w${C_RESET} `_ps1_git`"'

# Zsh
function precmd()
{
	export PROMPT="`_ps1_status` ${C_CYAN}%m ${C_LGREEN}%~${C_RESET} `_ps1_git`"
};
